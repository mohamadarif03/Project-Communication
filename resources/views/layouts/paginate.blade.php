<div class="flex justify-center mt-8">
    <ul class="pagination">
        ...
        @else
            <li>
                <a class="relative block py-2 px-3 leading-tight text-yellow-500 cursor-pointer mr-2"
                    onclick="GetData({{ $paginator->currentPage() - 1 }})" aria-label="Previous">
                    <span aria-hidden="true">&laquo;</span>
                </a>
            </li>
        @endif
                ...
                @foreach ($element as $page => $url)
                    @if ($page == $paginator->currentPage())
                        <li class="active"><span
                                class="relative block py-2 px-3 leading-tight cursor-pointer rounded-md bg-yellow-500 text-white mr-2">{{ $page }}</span>
                        </li>
                    @else
                        <li><a class="relative block py-2 px-3 leading-tight cursor-pointer bg-white border rounded-md border-gray-300 text-yellow-500 hover:bg-gray-200 mr-2"
                                onclick="GetData({{ $page }})">{{ $page }}</a></li>
                    @endif
                @endforeach
            @endif
        @endforeach
    ...
    @endif
        ...
        @if ($paginator->hasMorePages())
            <li>
                <a class="relative block py-2 px-3 leading-tight text-yellow-500 cursor-pointer hover:bg-gray-200 ml-2"
                    onclick="GetData({{ $paginator->currentPage() + 1 }})" aria-label="Next">
