    ...
    $.ajax({
        ...
        success:function(response){
            ...
            if(response.length > 0){
                    ...
                    var row = '<div class="col-span-1 w-full h-full px-2 py-1 mt-1 rounded-md bg-white">'+
                                '<div class="h-8 p-2 items-center relative w-full flex">'+
                                    ...
                                    '<ul id="dropdownlist-'+index+'" class="absolute dropdown-edit z-[1000] top-3 right-9 border-2 m-0 hidden min-w-max list-none overflow-hidden rounded-lg p-1 border-none bg-white bg-clip-padding text-left text-base shadow-lg">'+
                                        '<li>'+
                                            '<button id="btn-edit-'+index+'" data-name="'+data.name+'" data-email="'+data.email+'" data-role="'+data.role+'" onclick="edit('+data.id+')" class=" flex items-center" data-name="'+data.name+'" data-email="'+data.email+'" data-role="'+data.userrole+'" onclick="edit('+data.id+')" >'+
                ...
                var content = `<div class="w-full h-full col-span-3 mt-16 flex flex-col items-center justify-center">
                             <img src="../img/not-found.svg" class="w-1/4 mt-4" alt="">
